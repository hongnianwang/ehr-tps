...
#	parameters = json_obj['parameters']
	...
	if args.train[0] == args.test[0]:
		pd.DataFrame(eval_res).to_csv(  join(metrics_output, "Train_{}_metrics.csv".format(name) ) , index=False )
	else:
		pd.DataFrame(eval_res).to_csv(  join(metrics_output, "Test_{}_metrics.csv".format(name) ) , index=False )
